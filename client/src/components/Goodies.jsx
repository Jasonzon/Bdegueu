    ...
    return (
        <div className="goodhies">
            <div className="connection">
                <h1>Goodies</h1>
                {!(user && user.polyuser_role === "admin") ? null : <> {add ? <button onClick={() => setAdd(false)}>Annuler</button> : <button onClick={() => {setAdd(true);setModif(goodies.map((goodie) => false));setInputs2({name:"", price:"", pic:""});setImajo2({vide:true})}}>Ajouter</button>} </> }
            </div>
            ...
            <ul className={`good ${goodies.length !== 0 ? null : "none"}`}>
                {goodies.map(({goodies_name, goodies_pic, goodies_price, goodies_id},index) =>
                    <div className="goodies">
                        {!(user && user.polyuser_role === "admin") ? null : <> {del[index] ? <img onClick={() => delet(goodies_id)} className="trash-del" alt="trash" src={Trash} width="25" height="30"/> : <img onClick={() => setDel(goodies.map((tra,ind) => ind === index ? true : false))} className="trash" alt="trash" src={Trash} width="25" height="30"/>} </> }
